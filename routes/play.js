...
//         text: "Congrats! You completed Enigma.",
//         layout: "play_layout",
//       });
//     }
//     let last = false;
...
router.get("/", ensureLoggedIn(), async function (req, res, next) {
  try {
    ...
    const start = new Date("2023-03-05T10:00:00+05:30");
    if (curDateTime.getTime() > end.getTime()) {
      return res.render("end", { layout: "play_layout" });
    } else if (curDateTime.getTime() < start.getTime()) {
      return res.render("", { layout: "countdown" });
    ...
    const noOfQuestions = await QnA.countDocuments({});
    if (req.user.level > noOfQuestions) {
      return res.render("complete", {
        text: "Congrats! You completed Enigma.",
        layout: "play_layout",
      });
    ...
    let last = false;
    const cur_ques = req.user.level;
    const q1 = await QnA.findOne({ q_no: cur_ques }).lean();
    if (cur_ques > noOfQuestions) {
      last = true;
    }
    res.render("index", { q1, last, layout: "play_layout" });
  } catch (e) {
    next(e);
  }
});
...
router.post("/", ensureLoggedIn(), async function (req, res, next) {
  try {
    ...
    console.log(userAns.toLowerCase().replace(/\s/g, ""));
    let level = req.user.level;
    console.log(level, qno);
    const ques = await QnA.findOne({ q_no: qno });
    if (userAns == ques.answer && level === Number(qno)) {
      let fun = 1;
      await User.updateOne(
        { email: req.user.email },
        {
